    ...
    L.geoJson(data, {
      ...
      onEachFeature: (feature, layer) => {
        ...
        for (let prop in feature.properties) {
          pubContent += `<p>${prop.charAt(0).toUpperCase() + prop.slice(1)}: ${feature.properties[prop]}</p>`;
        }
        pubContent += '<button id="togglePlay">Toggle Play</button>';
        layer.bindPopup(pubContent);
        layer.on('popupopen', () => document.getElementById('togglePlay').addEventListener('click', window.onSpotifyWebPlaybackSDKReady));
      },
    }).addTo(map);
  });
