...
// Define the regular icon
let regularIcon = L.icon({
    iconUrl: '/static/icon.png',
    iconSize: [40, 40], // size of the icon
});

// Define the favourite icon
...
// Get the user's favourite pubs from the server
fetch('/user.favourite_pubs/')
    .then(response => response.json())
    .then(favouritePubs => {
        // Call addMarkersToMap with favouritePubs as an argument
        addMarkersToMap(data, favouritePubs);
    });
...
function addMarkersToMap(data) {
    L.geoJson(data, {
        pointToLayer: function (feature, latlng) {
            let isFavourite = user.favouritePubs.includes(feature.properties.id);
            let icon = isFavourite ? favouriteIcon : regularIcon;
