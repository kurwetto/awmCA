...
// Function to toggle the favorite status of a pub
function toggleFavorite(pubId) {
    // Send a POST request to toggle the favorite status of the pub
    fetch(`/toggle_favorite/${pubId}/`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRFToken': getCSRFToken() // Make sure to define this function to get the CSRF token
        }
    })
    .then(response => response.json())
    .then(data => {
        if (data.status === 'success') {
            // Update the icon based on the favorited status
            const pubIcon = document.getElementById(`pub-icon-${pubId}`);
            if (pubIcon) {
                pubIcon.src = data.favorited ? '/static/pub.png' : '/static/icon.png';
...
// Function to add buttons for favoriting pubs
function addFavoriteButtons(pubId) {
    const favoriteButton = document.createElement('button');
    favoriteButton.textContent = 'Favorite';
    favoriteButton.onclick = () => toggleFavorite(pubId);

    // Append the button to the pub content
    const pubContent = document.getElementById(`pub-content-${pubId}`);
    pubContent.appendChild(favoriteButton);
}
...
function addMarkersToMap(data) {
    L.geoJson(data, {
        ...
        onEachFeature: function (feature, layer) {
            const pubId = feature.id;
            let pubContent = `<div id="pub-content-${pubId}">`;

            // Add pub details to the popup content
            ...
            if (feature.properties.artist) {
                pubContent += `<p>Artist Performing: ${feature.properties.artist}</p>`;
            }
            if (feature.properties.songURL) {
                pubContent += `
                    <audio id="audio_${pubId}" src="${feature.properties.songURL}"></audio>
                    <button onclick="toggleAudio('audio_${pubId}')">Play Sample</button>
                `;
            }
            ...
            // Close the div for pub content
            pubContent += `</div>`;

            layer.bindPopup(pubContent);

            // Add favorite button to the popup content
            addFavoriteButtons(pubId);
