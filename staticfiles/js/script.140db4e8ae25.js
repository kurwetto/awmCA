...
function addMarkersToMap(data) {
    L.geoJson(data, {
        ...
        onEachFeature: function (feature, layer) {
            ...
            // Make sure that the geojson is only loaded once and not used again.
            // Create a seperate admin page to manage the map data.
...
// When the artist dropdown changes
$('#id_artist').change(function() {
    ...
    $.ajax({
        url: '/get_songs/',  // Update with the URL of your Django view
        data: {
            'artist': artistId
        },
        success: function(data) {
            // Clear the song dropdown
            $('#id_songURL').html('');

            // Add each song to the song dropdown
            data.forEach(function(song) {
                $('#id_songURL').append('<option value="' + song.id + '">' + song.name + '</option>');
            });
        }
    });
});