...
function isFavorited(pubId) {
    return fetch(`/is_favorited/${pubId}/`)
        .then(response => response.json())
        .then(data => data.isFavorited);
}
...
function addMarkersToMap(data) {
    L.geoJson(data, {
        pointToLayer: function (feature, latlng) {
            // Check if the pub is favorited
            let isFavorited = isFavorited(feature.properties.id);

            // Choose the icon based on whether the pub is favorited
            let icon = isFavorited ? favoriteIcon : defaultIcon;

            let marker = L.marker(latlng, { icon: icon, pubId: feature.properties.id });
